...
import { ArrowLeftRightIcon, ArrowRightIcon, ClockIcon } from "lucide-react";
...
export const SeatLayout = () => {
  ...
  const navigate=useNavigate();
   ...
   const handleSeatClick=(seatId)=>{
    if(!selectedTime) return toast("Please select your time first");
    ...
    <div key={row} className="flex-gap-2 mt-2">
      <div className="flex flex-wrap items-center justify-center gap-2">
        {Array.from({length:count},(_,i)=>{
          ...
          return (
            <button key={seatId} onClick={()=>handleSeatClick(seatId)} className={`h-8 w-8 rounded border border-primary/60 cursor-pointer ${selectedSeats.includes(seatId) && "bg-primary text-center"}`}>
              {seatId}
            </button>
          )
        })}
  ...
  return show ? (
    <div className="flex flex-col md:flex-row px-6 md:px-16 lg:px-40 py-30 md:pt-50 ">
      ...
      <div className="relative flex-1 flex flex-col items-center max-md:mt-16">
        ...
        items-center gap-1 mt-20 px-10 py-3 text-sm bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer active:scale-95">
          Proceed to checkout
          <ArrowRightIcon strokeWidth={3} className="w-4 min-h-4"/>
        </button>
      </div>
    </div>
  ) : (<Loading/>)
